...
2. Require `thread_id` and the reply `body`. If either is missing, ask a concise follow-up.
3. Call `reply_private_message_thread` directly.
4. If the client surfaces an authentication prompt or challenge, tell the user to complete Sanka sign-in and then retry.
5. Confirm the sent reply only after the tool succeeds.

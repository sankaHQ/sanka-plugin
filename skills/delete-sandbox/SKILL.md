---
description: Permanently delete a live Sanka sandbox workspace. Use when the user explicitly invokes /sakura:delete-sandbox.
...
1. Require the sandbox link id and explain that deletion permanently removes the sandbox workspace and everything inside it.
2. Call `delete_sandbox` with `confirm=false` for a preview unless the current request itself explicitly approves the destructive deletion; pass `confirm=true` only after that approval.
...
4. Report deletion as started only after the confirmed tool call succeeds, and clarify that production is not affected.
...
- This action is destructive: call `delete_sandbox` with `confirm=true` only after the user explicitly approves permanently deleting the sandbox workspace and everything inside it.
- Do not substitute the sandbox workspace id or 8-digit workspace code for the sandbox link id.
- Do not treat a preview response as a completed deletion.
- Do not use local repo files, terminal commands, Django shell, Postgres, or any repo-local fallback for live Sanka data.
- Do not call `search_docs` or `execute` when `delete_sandbox` covers the request.

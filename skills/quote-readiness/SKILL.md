...
Use only the attached Sakura MCP tools in this thread. This is a read-only Sanka workflow that checks Salesforce Opportunity quote readiness before any quote creation or workflow start.
...
3. Call `preview_workflow` with `workflow_type: "quote_readiness"` before any write. Treat this as Sanka's CPQ quote-readiness business workflow; Salesforce is the source platform. Summarize `ready`, `target_record`, `financials`, line item checks, `approval`, hard blockers, warnings, suggested fixes, Salesforce source records, Sanka records, and `orchestration_plan` fields such as would-reuse/create Company, Contact, Items, and platform mappings.
...
5. If the user explicitly asks to update Salesforce fields, only proceed when a dedicated Sakura MCP tool exposes safe writeback. Do not use generic code execution, local files, Salesforce MCP, or direct Salesforce APIs as a fallback.
6. If a Sakura MCP response includes `refresh_required`, `refresh_recommended`, or `suggested_user_facing_reply`, pause the workflow and show the refresh prompt before making further calls.
...
- If the direct tool call returns `Auth required`, `missing_scope`, or `insufficient_scope`, call `auth_status` exactly once with `{ required_scopes: ["deals:read","workflows:read"] }` to surface Connect Sanka metadata.
...
- Do not invent Salesforce or Sanka records, links, blockers, warnings, fixes, approval rules, or permission status that are not returned by Sakura MCP.

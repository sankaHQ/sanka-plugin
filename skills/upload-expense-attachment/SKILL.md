...
2. For a small, already available `content_base64` payload, call `upload_expense_attachment` directly.
3. For client-local PDFs or payloads that are too large or unreliable for one call, use `start_expense_attachment_upload`, then `append_expense_attachment_upload_chunk` with chunks at or below the returned `chunk_size` and `next_offset`, then `finish_expense_attachment_upload`.
4. Once `start_expense_attachment_upload` succeeds, finish the upload sequence. Multiple append calls are expected; do not abandon the attachment only because the upload needs many chunks.
5. If any upload tool returns `Auth required`, `missing_scope`, or `insufficient_scope`, call `auth_status` exactly once with `{ required_scopes: ["expenses:write"] }`. If it returns an explicit reconnect URL such as `connect_url` or `authorization_url`, show that URL verbatim. If it only returns OAuth metadata and not a reconnect URL, tell the user to launch the MCP client's native Sanka OAuth flow or reconnect action for this server, then retry the same Sanka request.
6. Summarize the uploaded result clearly, including the returned `file_id`.

---
name: list_expenses
description: List and inspect Sanka expenses using the read-only MCP expense tool.
---

# List expenses

Use this skill when the user wants to find or inspect expenses in Sanka.

## Scope

- Read-only expense retrieval
- Quick summaries from structured tool output
- Recent-expense review for the connected workspace
...
- For expense data requests, call `mcp__sanka_plugin__list_expenses` first. Do not call `auth_status` as a preflight.
- In Codex, a mistaken `auth_status` preflight can suppress the OAuth browser launch. The first expense read for a data request must be a protected `list_expenses` call.
- Never use `exec_command`, `read_thread_terminal`, local code search, `.env`, `manage.py shell`, `psql`, Django ORM, or any repo-local database access to answer a Sanka Plugin expense query.
...
1. Immediately call `mcp__sanka_plugin__list_expenses` for expense lookup requests. Do not call `mcp__sanka_plugin__auth_status` first.
...
4. If the protected expense tool itself is unavailable at call time, do not use any local fallback. In Codex this often means the thread loaded only the skill instructions, not the plugin attachment. Tell the user to start a new thread from `[@sanka-plugin](plugin://sanka-plugin@personal)` or the installed `Sanka Plugin` chip first. If that still fails, then ask them to reinstall or restart the plugin.
5. Call `mcp__sanka_plugin__list_expenses` with narrow arguments first:
   - `limit` for concise results
   - `workspace_id` only when the user explicitly needs a workspace override
   - `language` when localization matters
6. Return a short summary first, then list key expense records.
7. If auth errors appear, ask the user to reconnect instead of guessing.

## Output format

- Brief summary line
- Bullet list of top expenses with the most useful identifiers such as `description`, `company_name`, `amount`, `currency`, `status`, and `created_at`
- Optional note when the tool returned a limited subset of the available expenses

## Safety

- Do not perform create/update/delete actions.
- Do not fabricate fields that are not present in tool output.
- Do not substitute documentation search, terminal access, local repo inspection, Django shell, or raw SDK execution when the expense tool is missing.

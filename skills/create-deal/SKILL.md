---
description: Create deal in live Sanka. Use when the user explicitly invokes /sakura:create-deal.
...
2. Call `create_deal` directly.
3. If the direct tool call returns `Auth required`, `missing_scope`, or `insufficient_scope`, call `auth_status` exactly once with `{ required_scopes: ["deals:write"] }`. Include `required_user_facing_reply` verbatim when present; otherwise show `connect_url` verbatim. If neither is present, report that Connect Sanka could not be started and stop without attempting client-native OAuth. After the user connects, retry the same Sanka request.
4. Summarize the created deal and surface any returned ids or important fields.
...
- If the direct tool call returns `Auth required`, `missing_scope`, or `insufficient_scope`, call `auth_status` exactly once with `{ required_scopes: ["deals:write"] }` to surface Connect Sanka metadata.
...
- Do not call `search_docs` or `execute` when `create_deal` covers the request.

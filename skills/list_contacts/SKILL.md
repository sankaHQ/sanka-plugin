---
name: list_contacts
description: List and inspect Sanka contacts using the read-only CRM MCP contact tool.
---

# List contacts

Use this skill when the user wants to find or inspect contacts in Sanka CRM.

## Scope

- Read-only contact retrieval and filtering
...
- For contact data requests, call `mcp__sanka_plugin__list_contacts` first. Do not call `auth_status` as a preflight.
- In Codex, a mistaken `auth_status` preflight can suppress the OAuth browser launch. The first CRM read for a data request must be a protected `list_contacts` call.
- Never use `exec_command`, `read_thread_terminal`, local code search, `.env`, `manage.py shell`, `psql`, Django ORM, or any repo-local database access to answer a Sanka Plugin contact query.
...
1. Immediately call `mcp__sanka_plugin__list_contacts` for contact lookup requests. Do not call `mcp__sanka_plugin__auth_status` first.
...
5. Call `mcp__sanka_plugin__list_contacts` with narrow filters first:
...
- Brief summary line
- Bullet list of top contact matches with the most useful identifiers such as `name` and any obvious email fields

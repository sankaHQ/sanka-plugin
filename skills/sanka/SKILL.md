...
16. For expense create or update requests, attachments are optional when the user did not provide or require one. If the request includes a receipt, invoice, PDF, screenshot, or local attachment path, upload the original file before calling `create_expense` or `update_expense`. Use `upload_expense_attachment` for small, already available base64 payloads. For client-local PDFs or payloads that may truncate in one call, use `start_expense_attachment_upload`, append every chunk with `append_expense_attachment_upload_chunk` at or below the returned `chunk_size` and returned `next_offset`, then call `finish_expense_attachment_upload`. Multiple append calls are expected; do not abandon a user-provided or required attachment or write the expense without its `file_id` only because the upload needs many chunks.
...
- If the client only exposes a user-provided local expense attachment path, read only that exact file to produce base64 chunks for the upload. Do not read unrelated local files.

...
16. For delete, cancel, archive, or other destructive requests, ask for confirmation unless the user's confirmation is already explicit in the same request. Use `dry_run: true` for provider-side destructive checks when available.
17. For auth or connection requests, call `auth_status` once and surface reconnect instructions from the tool result.
18. For refresh, update, outdated plugin, or missing skill requests, stop live Sanka work and show the refresh prompt below.

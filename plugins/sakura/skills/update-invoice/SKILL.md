---
description: Update invoice in live Sanka. Use when the user explicitly invokes /sakura:update-invoice.
...
2. Call `update_invoice` directly.
...
4. Summarize the updated invoice and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_invoice` covers the request.

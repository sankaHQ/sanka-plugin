---
description: Delete inventory transaction from live Sanka. Use when the user explicitly invokes /sakura:delete-inventory-transaction.
...
2. Call `delete_inventory_transaction` directly.
...
- Do not call `search_docs` or `execute` when `delete_inventory_transaction` covers the request.

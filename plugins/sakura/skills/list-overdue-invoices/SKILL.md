---
description: List or search overdue invoices in live Sanka. Use when the user explicitly invokes /sakura:list-overdue-invoices.
...
2. Call `list_overdue_invoices` directly.
...
4. Summarize the returned overdue invoices clearly and do not mix in any local repo or database information.
...
- Do not call `search_docs` or `execute` when `list_overdue_invoices` covers the request.

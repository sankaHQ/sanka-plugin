---
description: List or search import jobs in live Sanka. Use when the user explicitly invokes /sakura:list-import-jobs.
...
2. Call `list_import_jobs` directly.
...
4. Summarize the returned import jobs clearly and do not mix in any local repo or database information.
...
- Do not call `search_docs` or `execute` when `list_import_jobs` covers the request.

---
description: Update estimate in live Sanka. Use when the user explicitly invokes /sakura:update-estimate.
...
2. Call `update_estimate` directly.
...
4. Summarize the updated estimate and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_estimate` covers the request.

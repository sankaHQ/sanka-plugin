---
description: Delete purchase order from live Sanka. Use when the user explicitly invokes /sakura:delete-purchase-order.
...
2. Call `delete_purchase_order` directly.
...
- Do not call `search_docs` or `execute` when `delete_purchase_order` covers the request.

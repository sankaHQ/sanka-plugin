---
description: Load one calendar bootstrap from live Sanka. Use when the user explicitly invokes /sakura:get-calendar-bootstrap.
...
2. Call `get_calendar_bootstrap` directly.
...
4. Summarize the returned calendar bootstrap clearly.
...
- Do not call `search_docs` or `execute` when `get_calendar_bootstrap` covers the request.

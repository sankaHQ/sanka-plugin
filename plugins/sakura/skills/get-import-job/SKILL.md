---
description: Load one import job from live Sanka. Use when the user explicitly invokes /sakura:get-import-job.
...
2. Call `get_import_job` directly.
...
4. Summarize the returned import job clearly.
...
- Do not call `search_docs` or `execute` when `get_import_job` covers the request.

---
description: Load one subscription from live Sanka. Use when the user explicitly invokes /sakura:get-subscription.
...
2. Call `get_subscription` directly.
...
4. Summarize the returned subscription clearly.
...
- Do not call `search_docs` or `execute` when `get_subscription` covers the request.

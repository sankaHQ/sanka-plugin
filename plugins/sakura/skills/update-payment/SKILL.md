---
description: Update payment in live Sanka. Use when the user explicitly invokes /sakura:update-payment.
...
2. Call `update_payment` directly.
...
4. Summarize the updated payment and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_payment` covers the request.

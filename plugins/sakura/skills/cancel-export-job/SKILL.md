---
description: Cancel export job in live Sanka. Use when the user explicitly invokes /sakura:cancel-export-job.
...
1. Require the target identifier and explicit cancel intent. If either is missing, ask a concise follow-up.
2. Call `cancel_export_job` directly.
...
4. Confirm the cancel result only after the tool succeeds.
...
- Do not call `search_docs` or `execute` when `cancel_export_job` covers the request.

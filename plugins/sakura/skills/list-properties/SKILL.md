---
description: List or search properties in live Sanka. Use when the user explicitly invokes /sakura:list-properties.
...
2. Call `list_properties` directly.
...
4. Summarize the returned properties clearly and do not mix in any local repo or database information.
...
- Do not call `search_docs` or `execute` when `list_properties` covers the request.

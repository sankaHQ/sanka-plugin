---
description: Load one bill from live Sanka. Use when the user explicitly invokes /sakura:get-bill.
...
4. Summarize the returned bill clearly.
...
- Do not call `search_docs` or `execute` when `get_bill` covers the request.

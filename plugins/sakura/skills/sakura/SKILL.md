...
Use only the attached Sakura MCP tools in this thread. This is the broad Sanka router for Claude Code and other clients that require a skill selection. Do not load every Sanka skill; classify the request and run the smallest matching Sakura MCP workflow or record action.
...
16. For expense create or update requests, attachments are optional when the user did not provide or require one. If the request includes a receipt, invoice, PDF, screenshot, or local attachment path, upload the original file before calling `create_expense` or `update_expense`. In local Sakura plugin clients, pass exact user-provided local receipt paths as `local_file_path` to `upload_expense_attachment`; the packaged local proxy reads the original bytes and forwards `content_base64` to hosted Sakura MCP. Use direct `content_base64` only when it is already available. For oversized local PDFs or when direct upload reports a size limit, use `start_expense_attachment_upload` with `local_file_path`, append chunks with `append_expense_attachment_upload_chunk` using `local_file_path`, the returned `next_offset`, and `local_chunk_size` at or below the returned `chunk_size`, then call `finish_expense_attachment_upload`. Multiple append calls are expected; do not abandon a user-provided or required attachment or write the expense without its `file_id` only because the upload needs many chunks.
17. For delete, cancel, archive, or other destructive requests, ask for confirmation unless the user's confirmation is already explicit in the same request. Use `dry_run: true` for provider-side destructive checks when available.
18. For auth or connection requests, call `auth_status` once and surface reconnect instructions from the tool result.
19. For refresh, update, outdated plugin, or missing skill requests, stop live Sanka work and show the refresh prompt below.
...
- If the needed Sakura MCP tool is unavailable, show the refresh prompt and stop instead of falling back to local repo files, terminal commands, Django shell, Postgres, HubSpot MCP, or generic web search.
- Do not invent business values, line items, commercial terms, approval outcomes, ids, or URLs that are not supplied by the user or returned by Sakura MCP.

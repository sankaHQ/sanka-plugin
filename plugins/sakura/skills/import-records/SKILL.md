---
description: Create a public import job in live Sanka. Use when the user explicitly invokes /sakura:import-records.
...
1. Require `file_id`. If it is missing, ask a concise follow-up.
2. Pass through import settings such as `object_type`, `operation`, `mapping_mode`, `key_field`, and `column_mappings` only when the user explicitly provides them.
3. Call `import_records` directly.
...
5. Summarize the created import job and its job id.

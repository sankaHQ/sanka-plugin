---
description: Update order in live Sanka. Use when the user explicitly invokes /sakura:update-order.
...
4. Summarize the updated order and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_order` covers the request.

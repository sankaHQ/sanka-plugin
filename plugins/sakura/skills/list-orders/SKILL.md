---
description: List or search orders in live Sanka. Use when the user explicitly invokes /sakura:list-orders.
...
4. Summarize the returned orders clearly and do not mix in any local repo or database information.
...
- Do not call `search_docs` or `execute` when `list_orders` covers the request.

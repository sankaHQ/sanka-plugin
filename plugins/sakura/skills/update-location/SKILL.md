---
description: Update location in live Sanka. Use when the user explicitly invokes /sakura:update-location.
...
2. Call `update_location` directly.
...
4. Summarize the updated location and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_location` covers the request.

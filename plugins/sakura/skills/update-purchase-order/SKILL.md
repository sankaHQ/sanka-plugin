---
description: Update purchase order in live Sanka. Use when the user explicitly invokes /sakura:update-purchase-order.
...
2. Call `update_purchase_order` directly.
...
4. Summarize the updated purchase order and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_purchase_order` covers the request.

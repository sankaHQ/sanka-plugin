---
description: Update inventory transaction in live Sanka. Use when the user explicitly invokes /sakura:update-inventory-transaction.
...
2. Call `update_inventory_transaction` directly.
...
4. Summarize the updated inventory transaction and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_inventory_transaction` covers the request.

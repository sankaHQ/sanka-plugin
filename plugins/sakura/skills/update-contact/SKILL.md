---
description: Update contact in live Sanka. Use when the user explicitly invokes /sakura:update-contact.
...
2. Call `update_contact` directly.
...
4. Summarize the updated contact and the changed fields clearly.
...
- Do not call `search_docs` or `execute` when `update_contact` covers the request.

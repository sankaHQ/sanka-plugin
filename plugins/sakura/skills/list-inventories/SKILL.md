---
description: List or search inventories in live Sanka. Use when the user explicitly invokes /sakura:list-inventories.
...
2. Call `list_inventories` directly.
...
4. Summarize the returned inventories clearly and do not mix in any local repo or database information.
...
- Do not call `search_docs` or `execute` when `list_inventories` covers the request.

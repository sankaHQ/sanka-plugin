...
- `proxy.mjs`: based on upstream `dist/proxy.js`, patched to keep tools attached before Sanka sign-in, suppress native OAuth challenges in tool results, and apply the local expense attachment bridge.
...
- Starting the native `mcp-remote` localhost OAuth coordinator during proxy boot creates stale `lock.json` and `code_verifier.txt` files when the client does not complete or persist that OAuth round-trip.
- The Sanka proxy therefore defers native OAuth coordination and suppresses tool-result `mcp/www_authenticate` metadata, leaving the hosted MCP `connect_url` in the normal tool result so agents can surface the stable Sanka connect link without losing the tool list.
